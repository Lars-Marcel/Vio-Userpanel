...
local function supportWindow(tickets)
	...
	addEventHandler("onClientGUIClick", supportWin.gridlist[1], function(btn, state)
		local ID = guiGridListGetItemText(supportWin.gridlist[1], guiGridListGetSelectedItem(supportWin.gridlist[1]), 1)
		if (tickets[ID]) then
...
local function showTicketWindow(tickets)
	...
	addEventHandler("onClientGUIClick", supportAdmin.gridlist[1], function(btn, state)
		local ID = guiGridListGetItemText(supportAdmin.gridlist[1], guiGridListGetSelectedItem(supportAdmin.gridlist[1]), 1)
		if (tickets[ID]) then
			local ticket = tickets[ID]
